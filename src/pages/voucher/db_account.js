...
const VoucherForm = () => {
  ...
  const [accountDetails, setAccountDetails] = useState([{ date: '', accountCode: '', accountName: '', credit: '' }]);
  ...
  const handleAddAccountDetail = () => {
    setAccountDetails([...accountDetails, { date: '', accountCode: '', accountName: '', credit: '' }]);
  };

  const fetchVouchers = async () => {
    try {
      const response = await fetch(`${apiBaseUrl}/debit_vouchers`);
  ...
  const resetForm = () => {
    ...
    setDbAccount('');
    setDescription('');
    setAccountDetails([{ date: '', accountCode: '', accountName: '', credit: '' }]);
  };
  ...
  const handleView = async (voucher) => {
    try {
      const response = await fetch(`${apiBaseUrl}/debit_vouchers/${voucher.id}`);
      const data = await response.json();
  ...
  const handleGenerateReport = async (voucher) => {
    try {
      ...
      doc.text(`Db. Account: ${data.db_account}`, 14, 40);
      ...
      data.accounts.forEach((acc, index) => {
        ...
        doc.text(acc.credit.toString(), 160, y);
        y += 8;
      });
      doc.setFontSize(12);
      doc.text(`Total Credit: ${data.total_credit.toFixed(2)}`, 14, y);
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`${apiBaseUrl}/debit_vouchers/${id}`, { method: 'DELETE' });
      await fetchVouchers();
    } catch (err) {
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ backgroundColor: 'primary.main' }}>
              ...
              <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>Db. Account</TableCell>
          ...
          <TableBody>
            {vouchers.map(voucher => (
              <TableRow key={voucher.id}>
                ...
                <TableCell>{voucher.db_account}</TableCell>
        ...
        <DialogContent>
          {selectedVoucher && (
            <>
              ...
              <TextField label='Db. Account' fullWidth margin='dense' value={selectedVoucher.db_account} disabled />
              ...
              <TableContainer component={Paper} sx={{ mt: 2 }}>
                ...
                <Typography variant="subtitle1" gutterBottom>
                  Total Credit: {selectedVoucher?.total_credit}
                </Typography>
              </TableContainer>
            </>
          )}
